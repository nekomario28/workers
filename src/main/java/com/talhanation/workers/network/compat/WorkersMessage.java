...
public interface WorkersMessage<T extends WorkersMessage<T>> extends Message<T> {
    ConcurrentHashMap<MethodKey, Method> METHOD_CACHE = new ConcurrentHashMap<>();
    Method ABSENT_METHOD = AbsentMethodHolder.SENTINEL;

    record MethodKey(Class<?> owner, String name, Class<?> parameterType) {
    }
    ...
    default void executeServerSide(WorkersNetworkContext context) {
    }

    default void executeClientSide(WorkersNetworkContext context) {
    }
    ...
    default void executeServerSide(IPayloadContext context) {
        invokeLegacy("executeServerSide", WorkersNetworkContext.class, new WorkersNetworkContext(context));
    }

    @Override
    default void executeClientSide(IPayloadContext context) {
        invokeLegacy("executeClientSide", WorkersNetworkContext.class, new WorkersNetworkContext(context));
    }
    ...
    private static Method resolveMethod(MethodKey key) {
        try {
            return key.owner().getMethod(key.name(), key.parameterType());
        } catch (NoSuchMethodException e) {
