...
public class WorkersNodeEvaluator extends NodeEvaluator {
    ...
    public void prepare(PathNavigationRegion region, Mob mob) {
        ...
        mob.setPathfindingMalus(PathType.WATER, WATER_CROSS_BASE);
        mob.setPathfindingMalus(PathType.WATER_BORDER, WATER_ADJACENT_MALUS);
    ...
    protected @NotNull Node getNode(int x, int y, int z) {
        ...
        Block below = this.currentContext.getBlockState(pos.below()).getBlock();
        if (below instanceof DirtPathBlock || below == Blocks.GRAVEL) {
    ...
    private int scanFallDepth(BlockPos pos) {
        ...
        for (int dy = 1; dy <= MAX_SAFE_FALL + 2; dy++) {
            ...
            PathType t = getBlockPathTypeStatic(this.currentContext.level(), m);
            if (t != PathType.OPEN && t != PathType.WATER && t != PathType.LAVA) {
                return depth;
            }
            depth++;
        }
    ...
    private boolean isWaterAdjacent(BlockPos pos) {
        ...
        for (Direction dir : Direction.Plane.HORIZONTAL) {
            ...
            if (this.currentContext.level().getFluidState(m).is(FluidTags.WATER)) {
                return true;
            }
        }
        return false;
    ...
    private int waterSpanAlongAxis(BlockPos pos, int dx, int dz) {
        ...
        for (int n = 1; n <= WATER_CROSS_MAX_PROBE; n++) {
            ...
            if (this.currentContext.level().getFluidState(m).is(FluidTags.WATER)) span++;
            else break;
        }
        // backward
        for (int n = 1; n <= WATER_CROSS_MAX_PROBE; n++) {
    ...
    private int countWalkableNeighbours(BlockPos pos) {
        ...
        for (Direction dir : Direction.Plane.HORIZONTAL) {
            m.set(pos.getX() + dir.getStepX(), pos.getY(), pos.getZ() + dir.getStepZ());
            PathType type = getBlockPathTypeStatic(this.currentContext.level(), m);
